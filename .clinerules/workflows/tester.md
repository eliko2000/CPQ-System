# Tester Workflow - CPQ System

**Purpose**: Write comprehensive tests using Vitest + React Testing Library for CPQ functionality, run test suites, analyze failures, and report coverage.

## When to Use This Workflow

- Writing tests for new CPQ features
- Ensuring code quality and preventing regressions
- Validating pricing calculations and quote processing
...
Use available tools for CPQ testing:

### Inspecting Pricing Data
```typescript
// Check component price history
// Use available database tools to query:
// /components?manufacturer_pn=eq.6ES7512-1DK01-0AB0&select=*,price_history(*)

// Verify assembly cost calculations
// /assemblies?id=eq.ASSEMBLY_ID&select=*,components(*)
```

### Setting Up Test Data
```typescript
// Insert test component with price history
// Use database tools to POST to /components with:
{
...
**Must Include:**
- Edge cases for pricing calculations (zero, negative, invalid values)
- Error handling for async operations
- Mock data that reflects real CPQ scenarios
- Tests for business logic constraints

**Avoid:**
- Testing implementation details instead of behavior
- Fragile tests that break with refactoring
- Tests that don't validate business rules
- Missing error case coverage

## When to Ask for Guidance

- Complex pricing scenarios not covered by existing patterns
- Unclear business logic requirements
- Test setup for external dependencies (OCR services, file uploads)
- Performance testing for large BOM calculations
- Integration testing across multiple services

## Success Metrics

- All critical pricing calculations have 100% test coverage
- No regressions in existing functionality
- Test execution time remains reasonable (< 30 seconds)
- Coverage goals met for all file categories
- Tests catch real bugs before they reach production

## Key Principle

CPQ tests must ensure pricing accuracy above all else. Financial errors are unacceptable. Every test should validate business rules and data integrity.
