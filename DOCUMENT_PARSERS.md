...
async function handleFileUpload(file: File): Promise<AIExtractionResult> {
  ...
  if (fileType.startsWith('image/')) {
    ...
    const { extractComponentsFromDocument } = await import('@/services/claudeAI');
...
async function processExtraction(result: AIExtractionResult) {
  ...
  const mediumConfidence = result.components.filter(c => c.confidence >= 0.4 && c.confidence < 0.7);
  ...
  showToast(`Imported ${highConfidence.length} components, ${mediumConfidence.length + lowConfidence.length} require review`, 'success');
}
```
