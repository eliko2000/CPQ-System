  ...
  const filteredComponents = useMemo(() => {
    ...
    return components.filter(
      comp =>
        ...
        comp.manufacturer
          ?.toLowerCase()
          .includes(componentSearchText.toLowerCase()) ||
  ...
  const getUniqueValues = useCallback(
    (field: string): string[] => {
      ...
      const values = currentQuotation.items
        .map(item => String(item[field as keyof QuotationItem] || ''))
        .filter(Boolean);
    ...
    () => [
      {
        ...
        cellClass: params =>
          params.data?.isSystemGroup ? 'ag-cell-bold' : 'ag-cell-right',
        valueGetter: (params: ValueGetterParams) => {
          if (params.data.isSystemGroup) {
            const system = currentQuotation?.systems.find(
              s => s.id === params.data.systemId
            );
              ...
              <div
                onClick={e => {
                  ...
                  const assembly = assemblies.find(
                    asm => asm.id === params.data.assemblyId
                  );
                  if (assembly) {
      ...
      },
      {
        ...
        cellClass: params =>
          params.data?.isSystemGroup ? '' : 'ag-cell-right',
        cellStyle: params => {
          if (params.data?.isSystemGroup) return {};
      ...
      },
      {
        ...
        width: 120,
        editable: (params: any) =>
          !params.data?.isSystemGroup && params.data?.itemType === 'labor',
        cellEditor: 'agSelectCellEditor',
        cellEditorParams: {
      ...
      },
      {
        ...
        filter: false,
        cellClass: params =>
          params.data?.isSystemGroup ? 'ag-cell-bold' : 'ag-cell-center',
        cellRenderer: (params: ICellRendererParams) => {
          if (params.data?.isSystemGroup) {
            return (
                    ...
                    <div className="mr-4 space-y-0.5 text-xs text-gray-500">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">
                          ₪
                          {calculations.totalEngineeringILS.toLocaleString(
                            'he-IL',
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">
                          ₪
                          {calculations.totalCommissioningILS.toLocaleString(
                            'he-IL',
                            { minimumFractionDigits: 0 }
                          )}
                        </span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono">
                          ₪
                          {calculations.totalInstallationILS.toLocaleString(
                            'he-IL',
                            { minimumFractionDigits: 0 }
                          )}
                        </span>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-sm">
                        {currentQuotation.systems.length}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-sm text-gray-600">
          ...
          >
            <div className="p-6 border-b border-gray-200">
              ...
              <Tabs
                value={selectorTab}
                onValueChange={v =>
                  setSelectorTab(v as 'components' | 'assemblies')
                }
              >
                          ...
                          >
                            ...
                            <div className="space-y-1 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-mono">
                                  ₪
                                  {component.unitCostNIS?.toLocaleString(
                                    'he-IL',
                                    { minimumFractionDigits: 2 }
                                  ) || '0.00'}
                                </span>
