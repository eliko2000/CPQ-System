...
export const QuotationStatisticsPanelSimplified = memo(
  function QuotationStatisticsPanelSimplified({
    statistics,
    className = '',
  }: QuotationStatisticsPanelSimplifiedProps) {
    // Get quotation type for context - memoized to avoid recalculation
    const quotationType = useMemo(
      () =>
        statistics.materialPercent > statistics.laborOnlyPercent + 20
          ? 'material-heavy'
          : statistics.laborOnlyPercent > statistics.materialPercent + 20
            ? 'labor-heavy'
            : 'balanced',
      [statistics.materialPercent, statistics.laborOnlyPercent]
    );

    // Warning for unusual ratios - memoized to avoid recalculation
    const warnings = useMemo(() => {
      const warns: string[] = [];
      if (statistics.laborPercent > 80)
        warns.push('אחוז עבודה גבוה מאוד (>80%)');
      if (statistics.materialPercent < 15)
        warns.push('אחוז חומרים נמוך מאוד (<15%)');
      if (statistics.profitByType.hardware.margin < 15)
        warns.push('מרווח חומרה נמוך מהמומלץ (<15%)');
      return warns;
    }, [
      statistics.laborPercent,
      statistics.materialPercent,
      statistics.profitByType.hardware.margin,
    ]);
      ...
      <div className={`space-y-6 ${className}`}>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
          ...
          <div className="mt-6">
            <div className="flex h-8 rounded-lg overflow-hidden shadow-sm">
              ...
              >
                {statistics.laborOnlyPercent > 15 &&
                  `${statistics.laborOnlyPercent.toFixed(0)}%`}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <div className="text-3xl font-bold text-gray-900">
              {statistics.componentCounts.total}
            </div>
            ...
            <div className="text-3xl font-bold text-green-700">
              {(
                (statistics.profitByType.hardware.margin *
                  ...
                  statistics.profitByType.software.margin *
                    statistics.softwarePercent +
                  statistics.profitByType.labor.margin *
                    statistics.laborPercent) /
                100
              ).toFixed(1)}
              %
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-center">
          <p className="text-xs text-blue-800">
            <strong>ייחוס לתעשייה:</strong> פרויקטי אינטגרציה רובוטיים טיפוסיים:
            16-33% חומרים, 67-84% עבודה
          </p>
        </div>
      </div>
    );
