...
vi.mock('ag-grid-react', () => ({
  AgGridReact: ({ rowData, _columnDefs, onGridReady }: any) => {
    ...
    return (
      <div data-testid="ag-grid">
        {rowData && rowData.map((row: any) => (
          <div key={row.id || row.displayNumber}>
            {row.componentName}
          </div>
        ))}
      </div>
...
vi.mock('../../../utils/currencyConversion', () => ({
  detectOriginalCurrency: (ils: number, usd: number, eur: number, stored?: string) => ({
    currency: stored || 'USD',
...
describe('QuotationEditor - Rendering and Initial State', () => {
  ...
  it('should show "Back to List" button', () => {
    render(<QuotationEditor />)
    expect(screen.getByText('חזור לרשימה')).toBeInTheDocument()
  })

  it('should show tabs for Items and Statistics', () => {
    render(<QuotationEditor />)
    expect(screen.getByText('פריטי הצעת מחיר')).toBeInTheDocument()
    expect(screen.getByText('סטטיסטיקה')).toBeInTheDocument()
  })
})
...
describe('QuotationEditor - Calculations', () => {
  ...
  it('should display system and item counts', () => {
    mockQuotation = createMockQuotation({
      systems: [
        { id: 'sys-1', name: 'System 1', order: 1, quantity: 1, createdAt: '2024-01-01' },
        { id: 'sys-2', name: 'System 2', order: 2, quantity: 1, createdAt: '2024-01-01' }
      ],
      items: [
        {
          id: 'item-1',
  ...
  it('should calculate system totals correctly', () => {
    mockQuotation = createMockQuotation({
      systems: [
        { id: 'sys-1', name: 'System 1', order: 1, quantity: 2, createdAt: '2024-01-01' }
      ],
      items: [
        {
          id: 'item-1',
...
describe('QuotationEditor - System Quantity Changes', () => {
  ...
  it('should multiply system items by system quantity', () => {
    mockQuotation = createMockQuotation({
      systems: [
        { id: 'sys-1', name: 'System 1', order: 1, quantity: 3, createdAt: '2024-01-01' }
      ],
      items: [{
        id: 'item-1',
        systemId: 'sys-1',
...
describe('QuotationEditor - Column Management', () => {
  ...
  it('should show column management button', () => {
    render(<QuotationEditor />)
    expect(screen.getByText('ניהול עמודות')).toBeInTheDocument()
  })
  ...
  it('should open column manager when clicked', async () => {
    ...
    const columnButton = screen.getByText('ניהול עמודות')
...
describe('QuotationEditor - User Interactions', () => {
  ...
  it('should navigate back when "Back to List" is clicked', async () => {
    ...
    const backButton = screen.getByText('חזור לרשימה')
  ...
  it('should switch to Statistics tab', async () => {
    ...
    const statsTab = screen.getByText('סטטיסטיקה')
    await user.click(statsTab)

    await waitFor(() => {
      expect(screen.getByText(/אין נתונים סטטיסטיים זמינים|סטטיסטיקה/)).toBeInTheDocument()
    })
  })
