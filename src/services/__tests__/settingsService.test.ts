...
import { loadSetting, saveSetting, migrateLocalStorageToSupabase } from '../settingsService'
...
describe('settingsService', () => {
  ...
  describe('loadSetting', () => {
    it('should load setting from cache', async () => {
      const testData = { categories: ['Test1', 'Test2'] }
      localStorage.setItem('cpq-settings-cache', JSON.stringify({
    ...
    it('should fallback to old localStorage format', async () => {
      const testData = { categories: ['Test1', 'Test2'] }
      localStorage.setItem('cpq-settings', JSON.stringify({
        componentCategories: testData
      }))
